...
function Card({ className, ...props }: React.ComponentProps<typeof View> & React.RefAttributes<View>) {
  return (
    <TextClassContext.Provider value="text-card-foreground">
      <View
        className={cn(
          'bg-card border-border flex flex-col gap-4 rounded-2xl border py-5 shadow-sm shadow-black/5',
          className
        )}
        {...props}
      />
...
function CardDescription({
  ...
  return <Text className={cn('text-muted-foreground text-base', className)} {...props} />;
}
